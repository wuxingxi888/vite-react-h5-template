...
以下规则属于默认基线，应在常规 React AI coding 任务中优先生效：
...
以下规则用于限定具体子领域的做法；当任务命中时应与基线规则一起使用，但不得反向覆盖基线：
...
2. 当前规则文件默认以 `alwaysApply: true` 维护，会在常规任务中自动应用
3. 详细示例与落地步骤见 `.agents/skills/` 目录下的技能文件
4. 当 rules 与 skills 冲突时，优先级以 [00-规则治理](./00-规则治理.md) 为准
5. 参考型技能（如 `vercel-*`）不属于默认基线，不可单独决定目录、依赖或实现方案
